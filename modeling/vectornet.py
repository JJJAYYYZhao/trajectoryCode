...
class NewSubGraph(nn.Module):
    ...
    def forward(self, input_list: list):
        ...
        hidden_states, lengths = merge_tensors(input_list, device, self.hidden_size)  # [bs,m,dim]
        ...
        hidden_states = self.layer_0(hidden_states)  # [bs,m,dim]
        hidden_states = self.layer_0_again(hidden_states)  # [bs,m,dim]
...
class SubGraph(nn.Module):
    def __init__(self, cfg, device):
        ...
        self.cfg = cfg
        self.device = device
        self.d_model = cfg.MODEL.hidden_size

    def forward(self, input_list):
        # input_list=list[bs]=[n,dim]  transfer list to vector with mask
        batch_size = len(input_list)
        input_states, lengths = merge_tensors(input_list, self.device, self.d_model)  # input_states=[bs,max_n,dim], len=list
        max_n = input_states.shape[1]

        # mask, mask the padding elements
        attn_mask = torch.zeros(batch_size, max_n, max_n).to(self.device)  # [bs,max_n,max_n]
        for i, len in enumerate(lengths):
            assert len > 0
            attn_mask[i, :len, :len].fill_(1)  # do not directly =1
